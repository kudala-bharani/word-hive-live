...
async function sbResetPlayerWordsForRound(roomCode) {
  const { error } = await supabaseClient
    .from('players')
    .update({ words_found: [] })
    .eq('room_code', roomCode);
...
function memResetPlayerWordsForRound(roomCode) {
  Object.values(players)
    .filter((p) => p.roomId === roomCode)
    .forEach((p) => {
      p.wordsFound = [];
...
const hasSupabaseClient = () => !!supabaseClient;

export async function createRoom(roomCode, hostName, duration) {
  if (hasSupabaseClient()) return sbCreateRoom(roomCode, hostName, duration);
  return memCreateRoom(roomCode, hostName, duration);
}

export async function getRoom(roomCode) {
  if (hasSupabaseClient()) return sbGetRoom(roomCode);
  return memGetRoom(roomCode);
}

export async function joinRoom(roomCode, playerName) {
  if (hasSupabaseClient()) return sbJoinRoom(roomCode, playerName);
  return memJoinRoom(roomCode, playerName);
}

export async function getRoomPlayers(roomCode) {
  if (hasSupabaseClient()) return sbGetRoomPlayers(roomCode);
  return memGetRoomPlayers(roomCode);
}

export async function updateRoomStatus(roomCode, status, updates = {}) {
  if (hasSupabaseClient()) return sbUpdateRoomStatus(roomCode, status, updates);
  return memUpdateRoomStatus(roomCode, status, updates);
}

export async function submitWord(roomCode, playerId, word, score) {
  if (hasSupabaseClient()) return sbSubmitWord(roomCode, playerId, word, score);
  return memSubmitWord(roomCode, playerId, word, score);
}

export async function getPlayer(playerId) {
  if (hasSupabaseClient()) return sbGetPlayer(playerId);
  return memGetPlayer(playerId);
}

export async function resetPlayerScores(roomCode) {
  if (hasSupabaseClient()) return sbResetPlayerScores(roomCode);
  return memResetPlayerScores(roomCode);
}

export async function resetPlayerWordsForRound(roomCode) {
  if (hasSupabaseClient()) return sbResetPlayerWordsForRound(roomCode);
  return memResetPlayerWordsForRound(roomCode);
}

export function subscribeToRoom(roomCode, callback) {
  if (hasSupabaseClient()) return sbSubscribeToRoom(roomCode, callback);
