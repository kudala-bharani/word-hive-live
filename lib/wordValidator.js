import { isAllowedWord } from './wordPolicy.js';
...
export function validateWord(word, centerLetter, availableLetters, validWords) {
  ...
  // Both the reviewed vocabulary and this hive's answer list must allow the word.
  if (!isAllowedWord(word) || !validWords.includes(upperWord.toLowerCase())) {
    return { valid: false, message: 'Word is not in this game\'s approved word list' };
  }
